(function (global, factory) {
    ...
    function handleAbsolutePath(thePath, content, showExeResult) {
        ...
        // 创建文件
        fs$5.writeFileSync(thePath, content);
        if (showExeResult) {
    ...
    function writePathFile({ path, content, showExeResult }) {
        ...
        try {
            ...
            // 写入文件
            fs$4.writeFileSync(path, content);
            msg = path + " 创建成功.";
            // 控制默认的日志打印
            if (showExeResult) {
                console.log(msg);
    ...
    */
    ...
    function getFileList({ dirPath, filesList, idv, ignoreList, needTypes, ignoreTypes }) {
        let id = 1;// id起始值
        var files = fs$2.readdirSync(dirPath);
    ...
    function readFileContent(paramsObj) {
        let { filePath, readEncode = 'utf-8', returnType = "string" } = paramsObj;
        const buffer = fs$1.readFileSync(filePath, readEncode);
    ...
    /**
        ...
        * @param {number} paramsObj.num 设置扫描多少次标识都不变，就认为是已经完成
        * @param {function} paramsObj.callBack 当认为执行过程结束后要执行的回调函数
        * @author zl-fire 2021/09/02
        * @example
        * 
      */
    function listnExePro(paramsObj = {}) {
        let {
        ...
        let backChangFlag = changFlag;//用于和标识变量比较的备份值
        let timer = setInterval(() => {
            if (changFlag != backChangFlag) {
                backChangFlag = changFlag;
            }
            if (num) {  //连续n秒都没有变化，认为已经结束
                clearInterval(timer);
                callBack();
            }
        }, msV);
